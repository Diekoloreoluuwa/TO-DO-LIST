...
def load_tasks():
    try:
        tasks = pickle.load(open("tasks.sav", "rb"))
        listbox_tasks.delete(0, tkinter. END)
        for task in tasks:
            listbox_tasks.insert(tkinter. END, task)
    except:
        tkinter.messagebox.showwarning(title = "WARNING!", message = "Cannot find a task in tasks.sav.")
   
 
def save_tasks():
    tasks = listbox_tasks.get(0, listbox_tasks.size())
    
    pickle.dump(tasks, open("tasks.sav", "wb"))
